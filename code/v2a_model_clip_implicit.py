...
from lib.model.v2a_spatial_ienc_implicit import V2A, Get_Clip_Enc ############################## # v2a, v2a_clip, v2a_spatial, v2a_spatial_ienc
...
class V2AModel(pl.LightningModule):
    ...
    def training_step(self, batch):
        ...
        clip_img_feature /= LA.norm(clip_img_feature, dim=-1, keepdim=True)
    ...
    def query_oc(self, x, cond):
        ...
        x = x.reshape(-1, 3)
        mnfld_pred = self.model.implicit_network(x, cond, torch.zeros(1,512).cuda())[:,:,0].reshape(-1,1)
    ...
    def query_od(self, x, cond, smpl_tfs, smpl_verts):
        ...
        output = self.model.implicit_network(x_c, cond, torch.zeros(1,512).cuda())[0]
        sdf = output[:, 0:1]
