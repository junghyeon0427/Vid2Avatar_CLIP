...
def main(opt):
    ...
    # mask = read_images_from_folder(opt.mask_dir)
    rgb_gt = read_images_from_folder_PIL(opt.rgb_gt_dir)
    rgb_pred = read_images_from_folder_PIL(opt.rgb_pred_dir)
    # pdb.set_trace()

    # for i in range(mask):
    #     mask_ = np.expand_dims(mask[i], -1)
    #     rgb_gt[i] = (rgb_gt[i] * mask)

    print(eval_metrics(rgb_gt, rgb_pred))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rgb_gt_dir', default='segmentations', type=str, help='rgb gt folder') # ../data/bike_csc/human_masked_gt
    parser.add_argument('--rgb_pred_dir', default='segmentations', type=str, help='rgb pred folder') # ../outputs/Video/bike_csc_clip/test_fg_rendering
